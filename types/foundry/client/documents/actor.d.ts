...
declare global {
	...
	> extends ActorConstructor {
		constructor(
			data: PreCreate<foundry.data.ActorSource>,
			context?: DocumentConstructionContext<Actor>,
		);
		...
		getActiveTokens(
			...
			document: true,
		): Embedded<NonNullable<TParent>>[];
		getActiveTokens(
			linked?: undefined,
			document?: undefined,
		): NonNullable<TParent['object']>[];
		getActiveTokens(
			linked?: boolean,
			document?: boolean,
